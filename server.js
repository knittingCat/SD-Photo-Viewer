...
const PID_FILE = path.join(os.homedir(), '.sd-photo-viewer.pid');

function cleanupPidFile() {
  try {
    if (fssync.readFileSync(PID_FILE, 'utf8').split(' ')[0] === String(process.pid)) fssync.unlinkSync(PID_FILE);
  } catch {
    // best-effort cleanup, ignore
  }
}
...
function listen(port, triesLeft) {
  const server = app.listen(port, HOST, (err) => {
    if (err) return;
    fssync.writeFileSync(PID_FILE, `${process.pid} ${port}\n`);
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE' && triesLeft > 0) {
      ...
      console.error(`Ports ${requestedPort()}–${port} are all in use. Try: sd-photo-viewer --port 8080`);
      process.exit(1);
    } else {
      throw err;
    }
